...
# Ensure modern TLS for API calls (Windows PowerShell can default to older TLS)
try {
...
if ($currentFont -notmatch "NSimSun|Gothic|Noto") {
    Write-Host "  Tip: To see all Unicode characters, set the terminal font to 'NSimSun'" -ForegroundColor DarkRed
    Write-Host
}

$Banner = @"

       _     ____________  ____            ____   ____    _____________   ____   ___    __ __________ ________    
      dM.    MMMMMMMMMMMP 6MMMMb/           6MMMMb/ `MM'    `MM`MMMMMMMMM  6MMMMb/ `MM    d' `MMMMMMMMM `MMMMMMMb.  
     ,MMb    /       dMP 8P    YM          8P    YM  MM      MM MM      \ 8P    YM  MM   d'   MM      \  MM    `Mb  
     d'YM.          dMP 6M      Y         6M      Y  MM      MM MM       6M      Y  MM  d'    MM         MM     MM  
    ,P `Mb         dMP  MM                MM         MM      MM MM    ,  MM         MM d'     MM    ,    MM     MM  
    d'  YM.       dMP   MM                MM         MMMMMMMMMM MMMMMMM  MM         MMd'      MMMMMMM    MM    .M9  
   ,P   `Mb      dMP    MM                MM         MM      MM MM    `  MM         MMYM.     MM    `    MMMMMMM9'  
   d'    YM.    dMP     MM                MM         MM      MM MM       MM         MM YM.    MM         MM  \M\    
  ,MMMMMMMMb   dMP      YM      6         YM      6  MM      MM MM       YM      6  MM  YM.   MM         MM   \M\   
  d'      YM. dMP       /8b    d9          8b    d9  MM      MM MM      / 8b    d9  MM   YM.  MM      /  MM    \M\  
_dM_     _dMMdMMMMMMMMMMM YMMMM9            YMMMM9  _MM_    _MM_MMMMMMMMM  YMMMM9  _MM_   YM._MMMMMMMMM _MM_    \M\_
...
Write-Host "Aqua" -ForegroundColor DarkRed
Write-Host ""
...
function Query-Modrinth {
    param([string]$Hash)
    try {
        $headers = @{ "User-Agent" = "AZC-Checker" }
        $versionInfo = Invoke-RestMethod -Uri "https://api.modrinth.com/v2/version_file/$Hash" -Method Get -Headers $headers -ErrorAction Stop
        if ($versionInfo.project_id) {
            $projectInfo = Invoke-RestMethod -Uri "https://api.modrinth.com/v2/project/$($versionInfo.project_id)" -Method Get -Headers $headers -ErrorAction Stop
...
function Query-Megabase {
    ...
    try {
        $headers = @{ "User-Agent" = "AZC-Checker" }
        $result = Invoke-RestMethod -Uri "https://megabase.vercel.app/api/query?hash=$Hash" -Method Get -Headers $headers -ErrorAction Stop
...
function Invoke-JvmScan {
    ...
    try {
        ...
        if (Get-Command Get-CimInstance -ErrorAction SilentlyContinue) {
            $cim = Get-CimInstance Win32_Process -Filter "ProcessId = $javaPid" -ErrorAction Stop
